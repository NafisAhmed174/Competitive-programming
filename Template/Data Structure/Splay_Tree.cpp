...
struct Pointer : basic_splay<Pointer> {
    int size = 1;
    Splay* node = nullptr;
 
    explicit Pointer(Splay* node) : node(node) {}
 
    static auto get_key(const Pointer* x) { return order_of_node(root, x->node); }
    static int get_size(const Pointer* x) { return x ? x->size : 0; }
